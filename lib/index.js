...
async function mapsApi(event) {
  ...
  try {
    ...
    if (!lodash.includes(['OK', 'ZERO_RESULTS'], fetchedContent.status)) {
      ...
      response: ${JSON.stringify(fetchedContent)}
      `);
    } else {
      const expireSeconds = process.env.EXPIRE_SECONDS;
      console.log(`
      ...
      cache-key: ${cacheKey},
      status: ${fetchedContent.status}
      cached: ${!lodash.includes(['ZERO_RESULTS'], fetchedContent.status)}
      response: ${JSON.stringify(fetchedContent)}
      `);
      if (path !== '/maps/api/timezone/json' && !lodash.includes(['ZERO_RESULTS'], fetchedContent.status)) {
