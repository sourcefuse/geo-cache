  ...
  api.get('/maps/api/*', async (ctx) => {
    ...
    const query = Object.keys(ctx.query)
      ...
      if(path === 'timezone/json') {
        query.timestamp = new Date(new Date().toLocaleDateString()).getTime() / 1000;
      }
    ...
    console.log(JSON.stringify({ url, query, urlString }));
    ...
    if (cachedContent) {
      console.log('hit', JSON.stringify({ url, sha, cacheKey }));
    ...
    if (res.status !== 200) {
      console.log('statusCode', url, res.status, res.statusText, query);
